...
### 使い方：
コメントを入れたいページを開き、拡張機能リストから 「easy comment」 のアイコンをクリックすると、コメントモードになります。　<br/>
コメントを付けたい範囲をドラッグで選択したら、コメント入力用のボックスが表示されるので、内容を入力する。コメントボックスはドラッグで任意の位置に移動可能。　<br/>
コメントボックス右上の「 ❌ 」ボタンを押すと範囲とコメントを削除します。
